...
step2: $(foreach trt, $(Traits), $(foreach cutoff, 2 3 4, $(foreach cis, 1e3 1e4 1e5, $(ODIR)/PARTITION_Ruzicka/$(trt)_$(cutoff)_$(cis).prs.gz)))

# % = $(GENO)/$(TRT)_$(CUTOFF)_$(CIS)
$(ODIR)/PARTITION_%.prs.gz:
	[ -d $(dir $@) ] || mkdir -p $(dir $@)
	Rscript make.prs.partition.R data/$(shell echo $* | awk -F'/' '{ print $$1 }')_Plink LD.info.txt data/GWAS/$(shell echo $* | awk -F'/' '{ print $$2 }' | awk -F'_' '{ print $$1 }').bed.gz data/gene_annot_limma_pval.tab.gz $(shell echo $* | awk -F'/' '{ print $$2 }' | awk -F'_' '{ print $$2 " " $$3 }')  $@
